...
public class SequenceLayout(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) :
    ...
    private fun placeDots() {
        ...
        stepsWrapper.children().forEachIndexed { i, view ->
            ...
            val layoutParams = FrameLayout.LayoutParams(16.toPx(), 16.toPx()) //TODO dynamic dot size
            val totalDotOffset = getRelativeTop(
                sequenceStep,
                stepsWrapper
            ) + sequenceStep.paddingTop + sequenceStep.getDotOffset() + 4.toPx() //TODO dynamic dot size
        ...
        foregroundLayoutParams.height = lastOffset - firstOffset - 12.toPx()
    ...
    fun getActiveItemScrollY(): Int =
        stepsWrapper.getChildAt(stepsWrapper.children().indexOfFirst { it is SequenceStep && it.isActive() }).top
